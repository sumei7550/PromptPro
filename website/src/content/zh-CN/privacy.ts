...
export const privacyContent: PrivacyContent = {
  hero: {
    ...
    updated: "最后更新：2026 年 9 月 5 日",
  },
  sections: [
    {
      title: "1. 概览",
      paragraphs: [
        "PromptPro 是一款提供 AI 提示词优化和可复用模板的浏览器扩展，可在支持的 AI 网站中使用，且不要求注册 PromptPro 账号。",
    ...
    },
    {
      title: "3. 提示词处理",
      paragraphs: [
        "当你请求 AI 优化时，PromptPro 会将当前提示词和最少的优化设置（语言、风格和目标平台）发送到 PromptPro API，再由该 API 转发给 DeepSeek。请求不包含页面 DOM、Cookie、完整对话历史、已保存提示词、本地优化历史或无关页面内容。",
        "AI 服务发生网络、超时、响应格式或服务异常时，PromptPro 可能使用扩展内置的本地规则回退。模板搜索和模板变量替换仍在扩展本地完成。",
    ...
    },
    {
      title: "5. 网站访问与权限",
      paragraphs: [
        "PromptPro 的内容脚本配置运行于 ChatGPT、Claude、Gemini、DeepSeek、豆包、Perplexity、Copilot、Grok、Google AI Studio、Cursor、v0 和 Lovable 等支持页面。主机权限用于让 PromptPro 功能在这些页面上运行，并允许扩展 Service Worker 调用 PromptPro API。",
    ...
    },
    {
      title: "6. 网络请求",
      paragraphs: [
        "AI 优化仅将当前提示词和最少请求设置发送到 PromptPro API 与 DeepSeek。服务端实现不会主动将提示词正文持久化到应用数据库，也不会将正文写入应用日志；DeepSeek 会依据其适用条款和数据处理规则处理请求。",
        "扩展不会随优化请求发送已保存模板、本地历史、设置集合、Cookie、页面 DOM、完整对话或无关页面内容，也不包含扩展分析或遥测传输。",
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "PromptPro 官网使用 Mixpanel 进行网站分析。网站分析用于了解页面浏览、经批准的来源分类、行动号召、Chrome Web Store 链接点击、语言切换、外部链接分类和 FAQ 互动。",
        "网站分析仅限于标准化页面路径、正式支持的语言值和受控的互动分类，不应包含提示词文本、模板文本、聊天内容、第三方 AI 网站内容、任意页面内容、表单值、键盘输入、剪贴板内容、邮箱地址或用户身份。",
        "官网和插件使用不同的事件命名空间和数据边界。官网事件使用 web_ 前缀；未来如需接入插件分析，必须单独审查并使用 extension_ 前缀。PromptPro 不会通过官网分析发送插件提示词、模板、历史、设置或使用状态。AI 处理请求属于产品功能调用，不是分析事件。",
        "Mixpanel 会接收客户端 IP，用于推导国家等基于 IP 的位置信息。PromptPro 不会将原始 IP 作为自定义事件属性添加。当前 Project 使用美国数据驻留区域。Session Replay、Heatmaps、表单采集、键盘输入采集和自动内容采集等可选功能不属于首版分析范围。",
      ],
    },
    {
      title: "8. 账号与个人信息",
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "你也可以通过 Chrome 的扩展数据管理功能或卸载扩展来移除本地扩展数据。根据 Chrome 的扩展存储行为，卸载扩展时会移除该扩展对应的 `chrome.storage.local` 数据。当前 PromptPro API 实现不会为优化请求创建应用数据库记录。",
      ],
    },
    {
      title: "11. 外部链接",
