...
export function FaqPage({ locale, content }: { locale: SupportedLocale; content: FaqContent }) {
  return <main id="main-content">
    ...
    <Section className={styles.faqSection}><div className={styles.categories}>{content.categories.map((category) => <Card key={category.title} variant="privacy" className={styles.category}><h2>{category.title}</h2><div className={styles.questions}>{category.questions.map((item) => <details key={item.question} className={styles.question}><summary>{item.question}<span className={styles.chevron} aria-hidden="true">+</span></summary><p>{item.answer}</p></details>)}</div></Card>)}</div></Section>
