...
export function SiteFooter({ locale }: SiteFooterProps) {
  ...
  return <footer className={styles.footer}><Container><div className={styles.footerGrid}><div className={styles.footerBrand}><Link className={styles.logo} href={base || "/"}><img src="/favicon.svg" alt="" width="32" height="32" /><span>PromptPro</span></Link><p>{c.description}</p><a className={styles.footerInstallLink} href={chromeWebStoreUrl}>{c.install}<span aria-hidden="true"> →</span></a><div className={styles.footerLegal}><Link href={`${base}/privacy`}>{c.privacy}</Link><span aria-hidden="true"> · </span>{unavailable(c.terms)}</div></div><FooterGroup title={c.product} links={[[features, `${base}/features`], [platforms, `${base}/platforms`], [templates, `${base}/templates`]]} unavailableLinks={[[pricing, `${base}/pricing`]]} unavailable={unavailable} /><FooterGroup title={c.resources} links={[]} unavailableLinks={[[faq, `${base}/faq`], [support, `${base}/support`], [changelog, `${base}/changelog`]]} unavailable={unavailable} /><FooterGroup title={c.company} links={[]} unavailableLinks={[[about, `${base}/about`]]} unavailable={unavailable} /></div><p className={styles.footerCopyright}>{c.copyright}</p></Container></footer>;
}
