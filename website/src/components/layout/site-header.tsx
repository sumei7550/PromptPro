...
export function SiteHeader({ locale, pathname = locale === "en" ? "/" : "/zh-CN" }: SiteHeaderProps) {
  ...
  const actions = <div className={styles.headerActions}><LanguageSwitcher locale={locale} pathname={pathname} label={c.language} languageNames={{ en: "English", "zh-CN": "简体中文" }} /><Button variant="primary" size="medium" href={chromeWebStoreUrl} aria-label={c.add}><ChromeIcon />{c.add}</Button></div>;
  return <header className={styles.header}><div className={styles.headerInner}><Link className={styles.logo} href={base || "/"} aria-label={`PromptPro ${en ? "home" : "首页"}`}><img src="/favicon.svg" alt="" width="28" height="28" /><span>PromptPro</span></Link><div className={styles.desktopNavigation}>{navigation(styles.navigation)}</div><div className={styles.desktopActions}>{actions}</div><MobileMenu openLabel={c.open} closeLabel={c.close}><div id="mobile-navigation" className={styles.mobileNavigation}>{navigation(styles.navigation)}{actions}</div></MobileMenu></div></header>;
}
