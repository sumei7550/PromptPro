...
- **v1.0.1（已迭代上线）**
  - 新增插件 Logo 图标（弹窗 Logo + 入口图标）
  - 新增首次使用新手引导，支持 AI 平台自动识别
  - 模板库支持全局搜索、中英文关键词模糊匹配与联想
  - 优化浮动按钮层级，修复 DeepSeek/Gemini 输入框遮挡问题
  - 免费次数耗尽弹窗提示，优化成功后正常扣减次数
  - 新增 `smartOptimizer.js` 核心优化引擎
  - 提示词优化模板迭代为 B 方案
  - 修复：次数统计异常、免费次数耗尽无响应、按钮遮挡
...
- **v1.0.1 (shipped)**
  - Added extension Logo icons (popup Logo + toolbar icon)
  - Added first-time onboarding guide with AI platform auto-detection
  - Template library supports global search & bilingual fuzzy matching
  - Fixed floating button z-index issue on DeepSeek/Gemini
  - Added quota exhaustion alert; fixed quota counting
  - Added `smartOptimizer.js` core engine
  - Upgraded prompt optimization to Version B template
  - Bug fixes: quota count, no response on zero quota, button occlusion
